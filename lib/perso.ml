...
let string_of_title ?(link = true) conf base and_txt p
...
let print_sosa conf base p link =
  ...
  if Sosa.gt sosa_num Sosa.zero then
    ...
    | Some ref ->
        ...
        in
        Output.printf conf
          "<img src=\"%s/sosa.png\" alt=\"sosa\" title=\"%s\"/> "
            ...
            x :=
              max !x
                (!GWPARAM_ITL.max_ancestor_level
                   conf base ip conf.bname max_lev level))
  in
  loop 0 ip;
  !x
...
let max_cousin_level conf base p =
  let max_lev =
    try int_of_string (List.assoc "max_cousins_level" conf.base_env)
    with Not_found | Failure _ -> default_max_cousin_lev
  in
...
and eval_simple_var conf base env ep = function
  | [ s ] -> (
      ...
      with Not_found -> str_val (eval_simple_str_var conf base env ep s))
  | _ -> raise Not_found
...
and eval_simple_bool_var conf base env =
  ...
  | s ->
      ...
      if v <> "" then
        ...
        let s = SrcfileDisplay.source_file_name conf v in
        Sys.file_exists s
      else raise Not_found

and eval_simple_str_var conf base env (_, p_auth) = function
  | "alias" -> (
      match get_env "alias" env with Vstring s -> s | _ -> raise Not_found)
  ...
  | "count1" -> (
      match get_env "count1" env with Vcnt c -> string_of_int !c | _ -> "")
  | "count2" -> (
      match get_env "count2" env with Vcnt c -> string_of_int !c | _ -> "")
  | "divorce_date" -> (
      match get_env "fam" env with
      | Vfam (_, fam, _, m_auth) when mode_local env -> (
  ...
  | "first_name_alias" -> (
      match get_env "first_name_alias" env with Vstring s -> s | _ -> "")
  | "fsources" -> (
      match get_env "fam" env with
  ...
  | "lazy_force" -> (
      ...
      | _ -> raise Not_found)
  | "level" -> (
      match get_env "level" env with Vint i -> string_of_int i | _ -> "")
  | "marriage_place" -> (
      match get_env "fam" env with
      | Vfam (_, fam, _, m_auth) when mode_local env ->
          if m_auth then
  ...
  | "nobility_title" -> (
      ...
      | Vtitle (p, t) ->
          if p_auth then string_of_title conf base (transl_nth conf "and" 0) p t
          else ""
      | _ -> raise Not_found)
  | "number_of_subitems" -> (
  ...
  | "on_marriage_date" -> (
      ...
      | Vfam (_, fam, _, m_auth) when mode_local env -> (
          ...
          | true, Some s -> (
              match p_getenv conf.base_env "long_date" with
              | Some "yes" ->
                  DateDisplay.string_of_ondate conf s
                  ^ DateDisplay.get_wday conf s
  ...
  | "reset_desc_level" -> (
      ...
      | _ -> raise Not_found)
  | "source_type" -> (
      match get_env "src_typ" env with Vstring s -> s | _ -> raise Not_found)
  | "surname_alias" -> (
...
and eval_person_field_var conf base env ((p, p_auth) as ep) loc = function
  ...
  | [ s ] -> (
      ...
      with Not_found -> str_val (eval_str_person_field conf base env ep s))
...
and eval_date_field_var conf d = function
  | [ "prec" ] -> (
      match d with
      | Dgreg (dmy, _) ->
          VVstring (Util.escape_html (DateDisplay.prec_text conf dmy))
      | _ -> VVstring "")
  | [ "day" ] -> (
      match d with
      | Dgreg (dmy, _) ->
          if dmy.day = 0 then VVstring "" else VVstring (string_of_int dmy.day)
      | _ -> VVstring "")
  | [ "day2" ] -> (
      match d with
      | Dgreg (dmy, _) -> (
  ...
  | [] ->
      VVstring
        (DateDisplay.string_of_date_aux ~link:false conf ~sep:"&#010;  " d)
  | _ -> raise Not_found
...
and eval_str_event_field conf base (p, p_auth)
  ...
  | "on_date" -> (
      ...
      | _ -> "")
  | "place" -> if p_auth then Util.string_of_place conf (sou base place) else ""
  | "note" ->
...
and eval_bool_person_field conf base env (p, p_auth) = function
  ...
  | "has_cremation_witnesses" ->
      ...
      p_auth && loop (events_list conf base p)
  | "has_death_date" -> (
      match get_death p with Death (_, _) -> p_auth | _ -> false)
  ...
  | "is_buried" -> ( match get_burial p with Buried _ -> p_auth | _ -> false)
  | "is_cremated" -> (
      match get_burial p with Cremated _ -> p_auth | _ -> false)
  | "is_dead" -> (
      match get_death p with
      | Death (_, _) | DeadYoung | DeadDontKnowWhen -> p_auth
      | _ -> false)
  | "is_certainly_dead" -> (
      match get_death p with OfCourseDead -> p_auth | _ -> false)
  | "is_descendant" -> (
...
and eval_str_person_field conf base env ((p, p_auth) as ep) = function
  ...
  | "auto_image_file_name" -> (
      match auto_image_file conf base p with Some s when p_auth -> s | _ -> "")
  ...
  | "birth_source" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_birth_src p))
  | "baptism_place" ->
  ...
  | "baptism_source" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_baptism_src p))
  | "burial_place" ->
  ...
  | "burial_source" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_burial_src p))
  | "child_name" ->
      let force_surname =
        match get_parents p with
        | None -> false
  ...
  | "death_source" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_death_src p))
  ...
  | "nb_families" -> (
      match get_env "p_link" env with
      | Vbool _ ->
          string_of_int (!GWPARAM_ITL.nb_families conf.command (get_iper p))
      | _ -> string_of_int (Array.length (get_family p)))
  | "notes" | "pnotes" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth conf.no_note (sou base (get_notes p))
  | "occ" ->
      if is_hide_names conf p && not p_auth then ""
      else string_of_int (get_occ p)
  | "occupation" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_occupation p))
  | "on_baptism_date" -> (
      match (p_auth, Adef.od_of_cdate (get_baptism p)) with
      | true, Some d -> (
          match p_getenv conf.base_env "long_date" with
  ...
  | "psources" ->
      let env = [ ('i', fun () -> Util.default_image_name base p) ] in
      get_note_source conf base env p_auth false (sou base (get_psources p))
  | "slash_burial_date" -> (
      match get_burial p with
      | Buried cod -> (
          match (p_auth, Adef.od_of_cdate cod) with
  ...
  | "prev_fam_father" -> (
      match get_env "prev_fam" env with
      | Vfam (_, _, (ifath, _, _), _) -> string_of_iper ifath
      | _ -> raise Not_found)
  | "prev_fam_index" -> (
      match get_env "prev_fam" env with
  ...
  | "prev_fam_mother" -> (
      match get_env "prev_fam" env with
      | Vfam (_, _, (_, imoth, _), _) -> string_of_iper imoth
      | _ -> raise Not_found)
  | "public_name" ->
...
and string_of_int_env var env =
  match get_env var env with Vint x -> string_of_int x | _ -> raise Not_found
...
let print_foreach conf base print_ast eval_expr =
  ...
  and print_simple_foreach env el al ini_ep ep efam loc = function
  ...
  and print_foreach_ancestor_level env el al ((p, _) as ep) =
    ...
    let rec loop gpl i n =
      if i > max_level then ()
        ...
        let n =
          List.fold_left
            (fun n gp -> match gp with GP_person (_, _, _) -> n + 1 | _ -> n)
            n gpl
        in
        let env =
          ...
          if name = Pevent Epers_Baptism then
            Array.iteri
              (fun i (ip, _) ->
                let p = pget conf base ip in
                let env =
                  ("baptism_witness", Vind p) :: ("first", Vbool (i = 0)) :: env
                in
                List.iter (print_ast env ep) al)
              wl
          else loop events
          ...
          if name = Pevent Epers_Birth then
            Array.iteri
              (fun i (ip, _) ->
                let p = pget conf base ip in
                let env =
                  ("birth_witness", Vind p) :: ("first", Vbool (i = 0)) :: env
                in
                List.iter (print_ast env ep) al)
              wl
          else loop events
          ...
          if name = Pevent Epers_Burial then
            Array.iteri
              (fun i (ip, _) ->
                let p = pget conf base ip in
                let env =
                  ("burial_witness", Vind p) :: ("first", Vbool (i = 0)) :: env
                in
                List.iter (print_ast env ep) al)
              wl
          else loop events
  ...
  and print_foreach_cell env al ep =
    ...
    Mutil.list_iter_first
      (fun first cell ->
        ...
        List.iter (print_ast env ep) al)
      celll
  and print_foreach_child env al ep = function
          ...
          if name = Pevent Epers_Death then
            Array.iteri
              (fun i (ip, _) ->
                let p = pget conf base ip in
                let env =
                  ("death_witness", Vind p) :: ("first", Vbool (i = 0)) :: env
                in
                List.iter (print_ast env ep) al)
              wl
          else loop events
  ...
  and print_foreach_source env al ((p, p_auth) as ep) =
    ...
    let srcl =
      if p_auth then
        ...
        in
        let buri_crem_lex =
          match get_burial p with Cremated _ -> "cremation" | _ -> "burial"
        in
        insert
  ...
  and print_foreach_witness env al ep = function
    | Vfam (_, fam, _, true) ->
        Array.iteri
          (fun i ip ->
            let p = pget conf base ip in
            let env = ("witness", Vind p) :: ("first", Vbool (i = 0)) :: env in
            List.iter (print_ast env ep) al)
          (get_witnesses fam)
    | _ -> ()
...
let gen_interp_templ ?(no_headers = false) menu title templ_fname conf base p =
  ...
  let env =
    ...
    let t_sosa =
      match sosa_ref with Some p -> init_sosa_t conf base p | _ -> None
    in
    ...
    let title h =
      ...
      if h then Output.print_string conf (simple_person_text conf base p true)
        ...
        Output.printf conf "<a href=\"%s%s\">%s</a>" (commd conf)
          (acces conf base p)
          (simple_person_text conf base p true)
    in
