...
let print_solution_ancestor conf base long p1 p2 pp1 pp2 x1 x2 list =
  let image_opt =
    match p_getenv conf.env "image" with Some "off" -> "&image=off" | _ -> ""
  in
  Output.print_string conf "<ul>\n";
  List.iter
    (fun (a, n) ->
    ...
    M.iter
      (fun ip (pp1, pp2, nn, nt, _) ->
        ...
        if nt > 1 && nn > 1 && nn < max_br then (
          ...
          in
          let border =
            match p_getenv conf.env "bd" with Some "on" -> "&bd=on" | _ -> ""
          in
  ...
  | (None, None, (x1, x2, _), _) :: _ when x1 = 0 || x2 = 0 ->
      ...
      Output.print_string conf
        (let s = person_title_text conf base p in
         Utf8.capitalize_fst
           (translate_eval (transl_a_of_b conf (transl conf "ancestors") s s)));
      Output.printf conf " %s"
...
let print_no_relationship conf base pl =
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "tree"))
  in
  Hutil.header conf title;
  Output.print_string conf "<ul>\n";
  List.iter
...
let print_multi conf base =
  ...
  let pl =
    ...
    loop [] 1
  in
  let lim = match p_getint conf.env "lim" with Some x -> x | None -> 0 in
