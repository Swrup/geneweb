...
let print_whole_notes conf base fnotes title s ho =
  ...
  Hutil.header_no_page_title conf (fun _ ->
      Output.print_string conf (if title = "" then fnotes else title));
  let what_links_page () =
    if fnotes <> "" then (
  ...
  in
  Output.printf conf "%s\n" s;
  (match ho with Some _ -> print_search_form conf (Some fnotes) | None -> ());
  Hutil.trailer conf
  ...
  List.iter
    (fun pg ->
      ...
      | Def.NLDB.PgFam ifam ->
          ...
          if conf.wizard then (
            Output.printf conf
              "<a class=\"mx-2\" href=\"%sm=MOD_FAM&i=%s&ip=%s&\">" (commd conf)
      ...
      | Def.NLDB.PgMisc fnotes ->
          ...
          Output.print_string conf "<tt>";
          if conf.wizard then (
            Output.printf conf "<a class=\"mx-2\" href=\"%sm=MOD_NOTES&f=%s&\">"
              (commd conf) fnotes;
            Output.print_string conf "<sup><i class=\"fa fa-cog\"></i></sup>";
            Output.print_string conf "</a>");
      ...
      | Def.NLDB.PgWizard wizname ->
          ...
          if conf.wizard then (
            Output.printf conf
              "<a class=\"mx-2\" href=\"%sm=MOD_WIZNOTES&f=%s&\">" (commd conf)
              (Mutil.encode wizname);
            Output.print_string conf "<sup><i class=\"fa fa-cog\"></i></sup>";
            Output.print_string conf "</a>");
...
let print_misc_notes conf base =
  ...
  let title h =
    Output.print_string conf
      (if d = "" then
       Utf8.capitalize_fst
         (Util.translate_eval (transl conf "miscellaneous notes"))
      else if h then "- " ^ d ^ " -"
      else "<tt>- " ^ d ^ " -</tt>")
  in
    ...
    List.iter
      (fun (r, f) ->
        ...
        | Some f ->
            ...
            Output.print_string conf "<tt>[";
            Output.printf conf "<a href=\"%sm=NOTES&f=%s\"%s>" (commd conf) f c;
