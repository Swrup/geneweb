...
let print_child_person conf base p =
  ...
  Output.printf conf "<label for=\"%s_fn\" class=\"mx-2 mb-0\">%s</label>" var
    (Utf8.capitalize_fst (transl_nth conf "first name/first names" 0));
  Output.print_string conf "</td>\n";
  Output.print_string conf "<td colspan=\"3\">\n";
  Output.printf conf
    "<input name=\"%s_first_name\" class=\"form-control\" size=\"23\" \
     maxlength=\"200\" id=\"%s_fn\" value=\"%s\">\n"
    var var
    (Util.escape_html first_name);
  Output.print_string conf "</td>\n";
  Output.printf conf "<td align=\"%s\">" conf.right;
  Output.printf conf "<label for=\"%s_occ\" class=\"mx-2 mb-0\">%s</label>" var
    (Utf8.capitalize_fst (transl conf "number"));
  Output.print_string conf "</td>\n";
  Output.print_string conf "<td>\n";
  Output.printf conf
    "<input class=\"form-control\" id=\"%s_occ\" name=\"%s_occ\" size=\"5\" \
     maxlength=\"8\"%s>\n"
    var var
  ...
  Output.printf conf "<tr align=\"%s\">\n" conf.left;
  Output.print_string conf "<td>";
  Output.printf conf "<label for=\"%s_sn\" class=\"mx-2 mb-0\">%s</label>" var
    (Utf8.capitalize_fst (transl_nth conf "surname/surnames" 0));
  Output.print_string conf "</td>\n";
  Output.print_string conf "<td colspan=\"5\">\n";
  Output.printf conf
    "<input name=\"%s_surname\" class=\"form-control\" size=\"40\" \
     maxlength=\"200\" id=\"%s_sn\" value=\"%s\">\n"
    var var surname;
  Output.print_string conf "</td>\n";
  Output.print_string conf "</tr>\n";
  Output.print_string conf "</tbody>\n";
...
let print_change conf base p =
  ...
  (let s = person_text conf base p in
   Output.print_string conf
     (Util.transl_a_of_b conf "" (reference conf base p s) s));
  ...
  Output.printf conf "<input type=\"hidden\" name=\"digest\" value=\"%s\">\n"
    digest;
  Output.print_string conf
    "<input type=\"hidden\" name=\"m\" value=\"CHG_CHN_OK\">\n";
...
let print_children_list conf base u =
  ...
  Output.print_string conf
    (Utf8.capitalize_fst (transl_nth conf "child/children" 1));
  Output.print_string conf "</h4>";
  Output.print_string conf "\n<p>\n";
  Output.print_string conf "<ul>\n";
  Array.iter
