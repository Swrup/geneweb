...
let print conf base p =
  ...
  title false;
  Output.print_string conf "</h2>\n";
  Output.printf conf "<form method=\"get\" action=\"%s\" class=\"mx-3 mb-3\">\n"
    conf.command;
  Util.hidden_env conf;
  Output.print_string conf
    "<input type=\"hidden\" name=\"m\" value=\"MRG_IND\">\n";
  ...
  Output.print_string conf
    "  <input type=\"radio\" class=\"custom-control-input\" name=\"select\" \
     id=\"input\" value=\"input\" checked>\n";
  Output.printf conf
    "  <label class=\"custom-control-label\" for=\"input\">%s</label>\n"
    (transl conf "any individual in the base");
  Output.print_string conf "</span>\n</span>\n";
  Output.print_string conf "<span class=\"col-auto\">\n";
  Output.printf conf
    "<input type=\"text\" class=\"form-control\" name=\"n\" \
     placeholder=\"%s.%s %s\" title=\"%s.%s %s\" size=\"50\" \
     id=\"inlineinput\" autofocus>\n\
     </span>\n"
    ...
    List.iter
      (fun p ->
        ...
        Output.printf conf
          ...
          (string_of_iper (get_iper p))
          (string_of_iper (get_iper p));
        Output.printf conf "  <label class=\"custom-control-label\" for=\"%s\">"
          (string_of_iper (get_iper p));
...
let print_possible_continue_merging conf base =
  ...
  | Some ini1, Some ini2 ->
      ...
      Output.printf conf {|<p><a href="%sm=MRG_IND&i=%s&i2=%s">%s</a>|}
        (commd conf) (string_of_iper ini1) (string_of_iper ini2)
  ...
  | _ -> (
      ...
      | Some ip ->
          ...
          if s1 <> "" || s2 <> "" then
            ...
            Output.printf conf
              {|<p><a href="%sm=MRG_DUP&ip=%s%s%s">%s</a> (%s)</p>|}
