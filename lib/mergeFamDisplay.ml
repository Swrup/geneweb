...
let print_differences conf base branches (ifam1, fam1) (ifam2, fam2) =
  let string_field title name proj =
    ...
    if x1 <> "" && x2 <> "" && x1 <> x2 then
      Output.printf conf
        "<h4>%s</h4><ul><li><input type=\"radio\" class=\"form-control\" \
         name=\"%s\" value=\"1\" checked>%s</li><li><input type=\"radio\" \
         class=\"form-control\" name=\"%s\" value=\"2\">%s</li></ul>"
  ...
  Output.print_string conf
    "<input type=\"hidden\" name=\"m\" value=\"MRG_FAM_OK\">\n";
  ...
  string_field (transl_nth conf "relation/relations" 0) "relation" (fun fam ->
      match get_relation fam with
  ...
  string_field
    ...
    ^ " / " ^ transl conf "place")
    "marriage_place"
    (fun fam -> sou base (get_marriage_place fam));
  string_field (transl conf "divorce") "divorce" (fun fam ->
      match get_divorce fam with
  ...
  Output.printf conf
    "</p><p><button type=\"submit\" class=\"btn btn-secondary \
     btn-lg\">%s</button></form>"
