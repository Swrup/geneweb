  ...
  | Some f when f <> "" ->
      let title _ =
        Output.print_string conf
          (Utf8.capitalize_fst (transl conf "clean history"))
      in
      Hutil.header conf title;
      Hutil.print_link_to_welcome conf true;
      Util.gen_print_tips conf
        (Utf8.capitalize_fst
           (transl conf "select the input you want to erase from the history"));
...
let print_clean_ok conf =
  ...
  | Some f when f <> "" ->
      let title _ =
        Output.print_string conf
          (Utf8.capitalize_fst (transl conf "history cleaned"))
      in
      Hutil.header conf title;
      Hutil.print_link_to_welcome conf true;
      let history = load_person_history conf f in
...
let string_of_title conf titles =
  let string_of_t_name t = match t.t_name with Tname s -> s | _ -> "" in
  let one_title t =
  ...
  in
  List.fold_left
    (fun accu t -> if accu = "" then one_title t else accu ^ ", " ^ one_title t)
    "" titles
...
and eval_simple_var conf base env (bef, aft, p_auth) = function
...
and eval_compound_var conf base env (bef, aft, p_auth) sl =
  let loop = function
    ...
    | [ "evar"; s ] -> (
        match p_getenv conf.env s with Some s -> s | None -> "")
    | "before" :: sl ->
...
and eval_gen_record conf base env (bef, aft, p_auth) = function
...
and eval_str_gen_record conf base env (bef, aft, p_auth) = function
  | "first_name" ->
      if p_auth then
  ...
  | "sex" ->
      if p_auth then
        let b =
          transl_nth conf "male/female/neuter" (Util.index_of_sex bef.gen_p.sex)
        in
        let a =
          transl_nth conf "male/female/neuter" (Util.index_of_sex aft.gen_p.sex)
        in
        diff_string b a
      else ("", "")
  | "access" ->
  ...
  | "birth" ->
      if p_auth then
        ...
        let a = string_of_cdate conf aft.gen_p.birth in
        diff_string b a
      else ("", "")
  | "birth_place" ->
      if p_auth then
  ...
  | "birth_note" ->
      if p_auth then
        ...
        let a = Util.escape_html aft.gen_p.birth_note in
        diff_string b a
      else ("", "")
  | "birth_src" ->
      if p_auth then
        let b = Util.escape_html bef.gen_p.birth_src in
        let a = Util.escape_html aft.gen_p.birth_src in
        diff_string b a
      else ("", "")
  | "baptism" ->
      if p_auth then
        let b = string_of_cdate conf bef.gen_p.baptism in
        let a = string_of_cdate conf aft.gen_p.baptism in
        diff_string b a
      else ("", "")
  | "baptism_place" ->
      if p_auth then
  ...
  | "baptism_note" ->
      if p_auth then
        ...
        let a = Util.escape_html aft.gen_p.baptism_note in
        diff_string b a
      else ("", "")
  | "baptism_src" ->
      if p_auth then
        let b = Util.escape_html bef.gen_p.baptism_src in
        let a = Util.escape_html aft.gen_p.baptism_src in
        diff_string b a
      else ("", "")
  | "death" ->
      if p_auth then
  ...
  | "death_note" ->
      if p_auth then
        ...
        let a = Util.escape_html aft.gen_p.death_note in
        diff_string b a
      else ("", "")
  | "death_src" ->
      if p_auth then
        let b = Util.escape_html bef.gen_p.death_src in
        let a = Util.escape_html aft.gen_p.death_src in
        diff_string b a
      else ("", "")
  | "burial" ->
      if p_auth then
  ...
  | "burial_note" ->
      if p_auth then
        ...
        let a = Util.escape_html aft.gen_p.burial_note in
        diff_string b a
      else ("", "")
  | "burial_src" ->
      if p_auth then
        let b = Util.escape_html bef.gen_p.burial_src in
        let a = Util.escape_html aft.gen_p.burial_src in
        diff_string b a
      else ("", "")
  | "pevent_name" -> (
      ...
      | Vpevent (bef, aft) ->
          if p_auth then
            ...
            | Some b, Some a ->
                ...
                let a = string_of_cdate conf a.epers_date in
                diff_string b a
      ...
      | Vpevent (bef, aft) ->
          if p_auth && not conf.no_note then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.epers_note in
                diff_string b a
      ...
      | Vpevent (bef, aft) ->
          if p_auth then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.epers_src in
                diff_string b a
  ...
  | "notes" ->
      if p_auth && not conf.no_note then
        ...
        let a = Util.escape_html aft.gen_p.notes in
        diff_string b a
      else ("", "")
  | "psources" ->
      if p_auth then
      ...
      | Vfam (bef, aft, m_auth) ->
          if m_auth then
            ...
            | Some b, Some a ->
                ...
                let a = string_of_cdate conf a.marriage in
                diff_string b a
      ...
      | Vfam (bef, aft, m_auth) ->
          if m_auth then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.marriage_src in
                diff_string b a
      ...
      | Vfevent (bef, aft, m_auth) ->
          if m_auth then
            ...
            | Some b, Some a ->
                ...
                let a = string_of_cdate conf a.efam_date in
                diff_string b a
      ...
      | Vfevent (bef, aft, m_auth) ->
          if m_auth && not conf.no_note then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.efam_note in
                diff_string b a
      ...
      | Vfevent (bef, aft, m_auth) ->
          if m_auth then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.efam_src in
                diff_string b a
      ...
      | Vfam (bef, aft, m_auth) ->
          if m_auth && not conf.no_note then
            ...
            | Some b, Some a ->
                ...
                let a = Util.escape_html a.comment in
                diff_string b a
...
and eval_simple_str_var conf base env (bef, aft, p_auth) = function
  | "acces" ->
  ...
  and print_foreach_family env xx al =
    ...
    let rec loop bef_f bef_c aft_f aft_c =
      ...
      | gen_f1 :: l1, gen_f2 :: l2 ->
          ...
          let env =
            ("fam", vfam)
            :: ("spouse_bef", Vstring sp1)
            :: ("spouse_aft", Vstring sp2)
            :: ("child", vchild) :: env
          in
          List.iter (print_ast env xx) al;
          loop l1 c1 l2 c2
        ...
        let rec loop i list =
          ...
          | gr :: l ->
              let env =
                ...
                :: ("wizard", Vstring gr.HistoryDiff.wizard)
                :: env
              in
              List.iter (print_ast env xx) al;
              loop (i + 1) l
