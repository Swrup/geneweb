...
let text_to conf = function
  | 0 ->
      transl_decline conf "specify" (transl_nth conf "generation/generations" 0)
...
let label_of_path paths p =
  let rec loop = function [] -> "" | c :: cl -> loop cl ^ String.make 1 c in
...
let print_repeat_child conf base p1 p2 e =
  ...
  if
    (get_sex p1 = Male && eq_istr (get_surname e) (get_surname p1))
    || (get_sex p2 = Male && eq_istr (get_surname e) (get_surname p2))
  then Output.print_string conf (person_text_without_surname conf base e)
...
let display_spouse conf base marks paths fam p c =
  ...
  if Gwdb.Marker.get marks (get_iper c) then
    Output.printf conf " (<tt><b>%s</b></tt>)" (label_of_path paths c)
...
let print_family conf base marks paths max_lev lev p =
  if lev <> 0 then
    Output.printf conf "<tt><b>%s</b></tt>.<br>" (label_of_path paths p);
    ...
    Array.iter
      (fun ifam ->
        ...
        if Gwdb.Marker.get paths c <> [] then
          let c = pget conf base c in
          Output.printf conf " => %s %s <tt><b>%s</b></tt>"
            (p_first_name base c) (p_surname base c) (label_of_path paths c))
      (get_family p)
...
let print_elem conf base paths precision (n, pll) =
  ...
  | [ [ p ] ] ->
      Output.printf conf "<strong>%s %s %s</strong>"
        ...
        (fun pl ->
          ...
          List.iter
            (fun p ->
              Output.print_string conf "<li>";
              Output.print_string conf "<strong>";
              wprint_geneweb_link conf (acces conf base p) (p_first_name base p);
              Output.print_string conf "</strong>";
...
let print_desc_table_header conf =
  ...
  Output.print_string conf "<th>\n";
  Output.print_string conf
    (Utf8.capitalize_fst (transl_nth conf "person/persons" 0));
  Output.print_string conf "</th>\n";
  ...
  if p_getenv conf.env "marr" = Some "on" then (
    ...
    incr nb_col;
    Output.print_string conf
      (Utf8.capitalize_fst (transl_nth conf "spouse/spouses" 1));
    Output.print_string conf "</th>\n");
  if p_getenv conf.env "marr_date" = Some "on" then (
    Output.print_string conf "<th>\n";
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "date of marriage"));
    incr nb_col;
    Output.print_string conf "</th>\n");
  ...
  if p_getenv conf.env "marr" = Some "on" then (
    ...
    (let u = p in
     if nb_families > 0 then (
       ...
       Output.printf conf " %s &nbsp;" (referenced_person_text conf base spouse))
    ...
    then
      ...
      for i = 1 to nb_families - 1 do
        ...
        if p_getenv conf.env "marr_place" = Some "on" then (
          ...
          (let mplace =
             if authorized_age conf base p && authorized_age conf base spouse
             then Util.string_of_place conf (sou base (get_marriage_place cpl))
             else ""
           in
           Output.printf conf "%s &nbsp;" mplace);
          Output.print_string conf "</td>\n");
...
let display_descendant_with_table conf base max_lev p =
  ...
  let rec loop lev nb_col first refl l =
    ...
    | (p, lab) :: q ->
        if first && lev > 0 && p_getenv conf.env "gen" = Some "on" then (
          ...
          Output.printf conf "%s %d"
            (Utf8.capitalize_fst (transl_nth conf "generation/generations" 0))
            lev;
          Output.print_string conf "</th>\n";
          Output.print_string conf "</tr>\n");
  ...
  (let nb_col = print_desc_table_header conf in
   loop 0 nb_col true [ (p, "") ] [ (p, "") ]);
  Output.print_string conf "</table>\n";
  Output.print_string conf "<p>\n";
  Output.printf conf "%s%s %d %s"
    (Utf8.capitalize_fst (transl conf "total"))
    (Util.transl conf ":") !nb_pers
    ...
    | Some (p, auth) when Array.length (get_family p) > 0 ->
        let rec loop tdl i =
          if i = Array.length (get_family p) then tdl
            ...
            let tdl = if i > 0 then (1, LeftA, TDtext "...") :: tdl else tdl in
            let td =
              let fam = foi base ifam in
