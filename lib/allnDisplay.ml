...
let print_alphabetic_big conf base is_surnames ini list len too_big =
  ...
  if not too_big then (
    ...
    Output.printf conf "<a href=\"%sm=%s&tri=A&o=A&k=%s\">" (commd conf) mode
      ini;
    Output.print_string conf (transl conf "long display");
    Output.print_string conf "</a>";
    Output.print_string conf "</li>\n";
    Output.print_string conf "<li>";
    Output.printf conf "<a href=\"%sm=%s&tri=S&o=A&k=%s\">" (commd conf) mode
      ini;
    Output.print_string conf (transl conf "short display");
    Output.print_string conf "</a>";
    Output.print_string conf "</li>\n";
    Output.print_string conf "<li>";
    Output.printf conf "<a href=\"%sm=%s&tri=S&o=A&k=%s&cgl=on\">" (commd conf)
      mode ini;
...
let print_alphabetic conf base is_surnames =
  ...
  if p_getenv conf.base_env "fast_alphabetic" = Some "yes" && ini = "" then (
  ...
  List.iter
    (fun (ini_k, l) ->
      ...
      Mutil.list_iter_first
        (fun first (s, cnt) ->
          let href =
            " href=\"" ^ commd conf ^ "m=" ^ mode ^ "&v=" ^ Mutil.encode s
            ^ "&t=A\""
          in
          let name = if first && need_ref then " id=\"a" ^ ini ^ "\"" else "" in
