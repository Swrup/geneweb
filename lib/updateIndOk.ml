...
let rec reconstitute_titles conf ext cnt =
  ...
  | Some t_ident, Some t_place, Some t_name ->
      ...
      let t =
        {
          t_name;
          t_ident = no_html_tags (only_printable t_ident);
...
let error_person conf err =
  if not conf.api_mode then (
    ...
    Output.print_string conf
      (Utf8.capitalize_fst @@ Update.string_of_error conf err);
    Output.print_string conf "\n";
...
let effective_mod ?prerr ?skip_conflict conf base sp =
  ...
  if List.assoc_opt "nsck" conf.env <> Some "on" then
...
let print_mod_ok conf base wl pgl p ofn osn oocc =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "person modified"))
  in
...
let print_add_ok conf base wl p =
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "person added"))
  in
  Hutil.header conf title;
  Hutil.print_link_to_welcome conf true;
  (* Si on a supprimé des caractères interdits *)
...
let print_del_ok conf =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "person deleted"))
  in
  Hutil.header conf title;
