        ...
        let rec loop1 prev pl =
          ...
          | x1 :: l1, x2 :: l2 ->
              if x1 = x2 then loop1 l1 l2
                ...
                List.iter
                  (fun _ -> Output.print_string conf "</ul></li>\n")
                  (x1 :: l1);
                loop1 [] (x2 :: l2))
          | _ -> assert false
        in
...
let print_all_places_surnames_short conf base ~add_birth ~add_baptism ~add_death
  ...
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "place"))
  in
  print_aux conf title (fun () ->
      let opt =
        print_aux_opt ~add_birth ~add_baptism ~add_death ~add_burial
          ~add_marriage
      in
      Output.printf conf "<p><a href=\"%sm=PS%s&display=long\">%s</a></p><p>"
        (commd conf) opt
        (transl conf "long display");
      let last = Array.length array - 1 in
      Array.iteri
        (fun i (s, x) ->
          Output.printf conf "<a href=\"%sm=PS%s&k=%s\">%s</a> (%d)%s"
            (commd conf) opt (Mutil.encode s) s x
            (if i = last then "" else ",\n"))
        array;
...
let print_all_places_surnames_long conf base ini ~add_birth ~add_baptism
  ...
  let title _ =
    Output.printf conf "%s / %s"
      (Utf8.capitalize_fst (transl conf "place"))
