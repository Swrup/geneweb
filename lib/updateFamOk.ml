...
let reconstitute_somebody conf var =
  ...
  let first_name, surname =
    ...
    if
      List.exists contain_fn Name.forbidden_char
...
let reconstitute_from_fevents (nsck : bool) (empty_string : 'string)
  ...
  let marr =
    if nsck then
      ...
      let relation =
        match relation with Married -> NoSexesCheckMarried | x -> x
      in
...
let reconstitute_family conf base nsck =
  ...
  let origin_file =
    match p_getenv conf.env "origin_file" with Some x -> x | None -> ""
  in
  let fam_index =
...
let error_family conf err =
  ...
  |> Utf8.capitalize_fst |> Output.print_string conf;
...
let check_parents conf cpl =
  let check get i =
    ...
    if fn = "" then
      if sn <> "" then
        Some
          (Update.UERR_missing_first_name (transl_nth conf "father/mother" i))
      else None
    else if sn = "" then
...
let print_error_disconnected conf =
  let err =
    Update.UERR (transl conf "msg error disconnected" |> Utf8.capitalize_fst)
  in
  Update.prerr conf err @@ fun () ->
...
let patch_person_with_pevents base ip =
  ...
  let evt_birth =
    ...
    in
    if Adef.od_of_cdate p.birth <> None then evt ~date:p.birth ()
  ...
  let evt_baptism =
    let evt ?date () =
      ...
      Some (evt ~name ?date ~place ~note ~src ())
    in
    if Adef.od_of_cdate p.baptism <> None then evt ~date:p.baptism ()
  ...
  let evt_death =
    ...
    | Death (_, cd) ->
        ...
        evt ~date ()
    | _ -> if sou base p.death_place = "" then None else evt ()
  in
    ...
    let rec loop pevents accu =
      ...
      | [] ->
          ...
          let accu =
            ...
            else match evt_death with Some evt -> evt :: accu | None -> accu
          in
          List.rev accu
      | evt :: l -> (
...
let effective_inv conf base ip u ifam =
  let rec loop = function
    ...
    | _ ->
        ...
        raise
        @@ Update.ModErr (Update.UERR (__FILE__ ^ " " ^ string_of_int __LINE__))
  in
...
let print_mod_ok conf base (wl, ml) cpl des =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "family modified"))
  in
...
let print_add_ok conf base (wl, ml) cpl des =
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "family added"))
  in
  Hutil.header conf title;
  Hutil.print_link_to_welcome conf true;
  (* Si on a supprimé des caractères interdits *)
...
let print_del_ok conf base wl =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "family deleted"))
  in
  Hutil.header conf title;
  Hutil.print_link_to_welcome conf true;
...
let print_inv_ok conf base p =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "inversion done"))
  in
  Hutil.header conf title;
  Hutil.print_link_to_welcome conf true;
...
let print_add o_conf base =
  ...
  let redisp =
    match p_getenv conf.env "return" with Some _ -> true | _ -> false
  in
  let digest =
    match p_getenv conf.env "ip" with
    | Some ip ->
