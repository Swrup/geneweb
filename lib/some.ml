...
let not_found conf txt x =
  let title _ =
    Output.printf conf "%s: \"%s\"" (Utf8.capitalize_fst txt)
      (Util.escape_html x)
  in
...
let print_branch_to_alphabetic conf x nb_branch =
  ...
  Output.print_string conf "</td>";
  Output.print_string conf "<td>";
  Output.printf conf "<img src=\"%s/%s\" alt=\"\" title=\"\">\n"
    ...
    (fun first x ->
      ...
      if is_surname then
        Output.printf conf "%s%s"
          (surname_without_particle base p)
          (surname_particle base p)
  ...
  | Some (txt, sel) ->
      let req =
        List.fold_left
          ...
          "" conf.env
      in
      Output.printf conf "<a id=\"if%s\" href=\"%s%s%s%s\" rel=\"nofollow\">"
        txt (prefix_base conf) req
...
let print_branch conf base psn name =
  ...
  let rec loop p =
    ...
    in
    let first_select =
      match family_list with (_, _, select) :: _ -> select | _ -> None
    in
    let print_elem p with_link with_id with_sn =
      let hl = if with_link then "strong" else "em" in
      let render p =
        if with_link then
   ...
   (let _ =
      List.fold_left
        (fun n bh ->
          ...
          Output.printf conf "<a href=\"%sm=N&v=%s&br=%d\" rel=\"nofollow\">"
