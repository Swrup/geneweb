...
let print_differences conf base branches p1 p2 =
  let gen_string_field chk1 chk2 title name proj =
    ...
    if x1 <> "" && x1 <> "?" && x2 <> "" && x2 <> "?" && x1 <> x2 then (
      ...
      Output.printf conf
        "  <input class=\"custom-control-input\" type=\"radio\" id=\"%s1\" \
         name=\"%s\" value=\"1\"%s>\n"
        name name chk1;
      Output.printf conf
        "  <label class=\"custom-control-label\" for=\"%s1\">%s</label>\n" name
        x1;
      Output.print_string conf "</div>\n";
      Output.print_string conf
        "<div class=\"custom-control custom-radio ml-3 mb-2\">\n";
      Output.printf conf
        "  <input class=\"custom-control-input\" type=\"radio\" id=\"%s2\" \
         name=\"%s\" value=\"2\"%s>\n"
        name name chk2;
      Output.printf conf
        "  <label class=\"custom-control-label\" for=\"%s2\">%s</label>\n" name
        x2;
      Output.print_string conf "</div>\n")
  in
  ...
  Output.print_string conf
    "<input type=\"hidden\" name=\"m\" value=\"MRG_IND_OK\">\n";
  ...
  string_field (transl_nth conf "surname/surnames" 0) "surname" (fun p ->
      p_surname base p);
  (let select_smallest_num = p_first_name base p1 = p_first_name base p2 in
   gen_string_field
     ...
     (if get_occ p1 > get_occ p2 && select_smallest_num then " checked" else "")
     (transl conf "number") "number"
     (fun p -> string_of_int (get_occ p)));
  string_field (transl_nth conf "image/images" 0) "image" (fun p ->
  ...
  string_field (transl conf "public name") "public_name" (fun p ->
  ...
  string_field (transl conf "sex") "sex" (fun p ->
      match get_sex p with Male -> "M" | Female -> "F" | Neuter -> "");
  string_field (transl conf "birth") "birth" (fun p ->
      match Adef.od_of_cdate (get_birth p) with
      | None -> ""
      | Some d -> DateDisplay.string_of_ondate conf d);
  string_field
    (transl conf "birth" ^ " / " ^ transl conf "place")
    "birth_place"
    (fun p -> sou base (get_birth_place p));
  string_field (transl conf "baptism") "baptism" (fun p ->
      match Adef.od_of_cdate (get_baptism p) with
      | None -> ""
      | Some d -> DateDisplay.string_of_ondate conf d);
  string_field
    (transl conf "baptism" ^ " / " ^ transl conf "place")
    "baptism_place"
    (fun p -> sou base (get_baptism_place p));
  string_field (transl conf "death") "death" (fun p ->
      let is = 2 in
  ...
  string_field
    (transl conf "death" ^ " / " ^ transl conf "place")
    "death_place"
    (fun p -> sou base (get_death_place p));
  string_field (transl conf "burial") "burial" (fun p ->
      let is = 2 in
      ...
      | Cremated cod -> (
          ...
          | Some d -> " " ^ DateDisplay.string_of_ondate conf d));
  string_field
    (transl conf "burial" ^ " / " ^ transl conf "place")
    "burial_place"
    (fun p -> sou base (get_burial_place p));
  Output.printf conf
  ...
  if branches <> [] then (
    ...
    List.iter
      (fun (ip1, ip2) ->
        ...
        Output.printf conf "<tr align=\"%s\"><td>%s%s</td><td>%s%s</td></tr>"
          conf.left
...
let propose_merge_fam conf base branches fam1 fam2 p1 p2 =
  ...
  Output.printf conf "%s%s\n"
    ...
    (transl conf ":");
  Output.printf conf "<ul><li><a href=\"%s%s\">" (commd conf)
    (acces conf base p1);
  MergeDisplay.print_someone conf base p1;
  Output.printf conf "</a> %s <a href=\"%s%s\">" (transl conf "with")
    (commd conf) (acces conf base p2);
  MergeDisplay.print_someone conf base p2;
...
let different_sexes conf base p1 p2 =
  ...
  Output.printf conf {|<a href="%s%s">%s</a>|} (commd conf) (acces conf base p1)
    (Gutil.designation base p1);
  Output.print_string conf "</li><li>";
  Output.printf conf {|<a href="%s%s">%s</a>|} (commd conf) (acces conf base p2)
    (Gutil.designation base p2);
...
let print_merged conf base wl p =
  ...
  | Some "MRG_DUP_IND_Y_N", Some ip ->
      ...
      in
      Output.print_string conf "<p>\n";
      Output.printf conf "<a href=%sm=MRG_DUP&ip=%s%s%s>" (commd conf)
