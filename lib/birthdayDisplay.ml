...
let propose_months conf mode =
  ...
  for i = 1 to 12 do
    ...
    Output.print_string conf
      (Utf8.capitalize_fst
         (Util.translate_eval (transl_nth conf "(month)" (i - 1))));
...
let gen_print_menu_birth conf base f_scan mode =
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "birthdays"))
  in
  ...
  print_birth_day conf base
    (transl conf "the day after tomorrow")
    (ftransl conf "%s, it will be %s of")
...
let print_menu_marriage conf base =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "anniversaries of marriage"))
  in
  let tom = day_after conf.today in
  let aft = day_after tom in
  let list_tod = ref [] in
