...
and eval_simple_variable conf = function
  ...
  | "prefix_no_pmod" ->
      ...
      in
      let c = conf.command ^ "?" in
      List.fold_left (fun c (k, v) -> c ^ k ^ "=" ^ v ^ "&") c (henv @ conf.senv)
  | "prefix_no_wide" ->
      let henv =
        List.fold_right
  ...
  | "prefix_no_all" ->
      let henv =
        List.fold_right
          (fun (k, v) henv ->
            if k = "templ" || k = "p_mod" || k = "wide" then henv
            else (k, v) :: henv)
          conf.henv []
      in
      let c = conf.command ^ "?" in
...
let print_error conf ((fname, bp, ep) as pos) exc =
  ...
  if !nb_errors <= 10 then (
    ...
    let line = if fname = "" then None else line_of_loc conf pos in
    Printf.eprintf ", ";
    (match line with
    | Some (lin, col1, col2) ->
    ...
    | Some v ->
        {
          conf with
          env =
            Mutil.list_replace ("with_logo", v) ("with_logo", "yes") conf.env;
        }
...
and print_var print_ast_list conf ifun env ep loc sl =
  let rec print_var1 eval_var sl =
    ...
    with Not_found -> (
      match sl with
      | [ "include"; templ ] -> (
          match Util.open_templ_fname conf templ with
          | Some (_, fname) -> (
...
and print_variable conf sl =
  try Output.print_string conf (eval_variable conf sl)
  with Not_found -> (
    try
      match sl with
