...
let print_mod_ok conf base =
  ...
  let new_input =
    Opt.map_default "" only_printable (p_getenv conf.env "nx_input")
  in
  ...
  if nb_pers <> 0 && data_modified then (
    ...
    Output.printf conf "%s%s %d "
      (Utf8.capitalize_fst (transl conf "modification successful"))
      (Util.transl conf ":") (min nb_pers max_updates);
    ...
    if nb_pers > max_updates then (
      ...
      Output.printf conf {|<input type="hidden" name="key" value="%s">|}
        (List.assoc "key" conf.env);
      ...
      Output.printf conf
        {|<input type="hidden" name="nx_input" size="80" maxlength="200" value="%s" id="data">|}
    ...
    Output.printf conf
      {|<p><a href="%sm=MOD_DATA&data=%s&s=%s" id="reference">%s</a></p>|}
      (commd conf) data ini
    ...
    let title _ =
      Output.print_string conf
        (Utf8.capitalize_fst (transl conf "no modification"))
    in
    Hutil.header conf title;
    Hutil.print_link_to_welcome conf true;
    Output.printf conf
