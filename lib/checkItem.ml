...
let strictly_younger age year =
  match age.prec with After -> false | _ -> age.year < year

let strictly_older age year =
  match age.prec with Before -> false | _ -> age.year > year

let odate = function Some (Dgreg (d, _)) -> Some d | _ -> None
...
let compare_event_name name1 name2 =
  ...
  | ( Psort Epers_Baptism,
      Psort (Epers_Death | Epers_Funeral | Epers_Burial | Epers_Cremation) ) ->
      -1
  | ( Psort (Epers_Death | Epers_Funeral | Epers_Burial | Epers_Cremation),
      Psort Epers_Baptism ) ->
      1
...
let check_order_pfevents get_name get_date warning events =
  ...
  let rec loop = function
    | e1 :: e2 :: events -> (
        ...
        | Psort (Epers_Name _) | Fsort (Efam_Name _) -> loop (e2 :: events)
        | n1 -> (
            match get_name e2 with
            | Psort (Epers_Name _) | Fsort (Efam_Name _) -> loop (e1 :: events)
            | n2 ->
                if compare_event_name n1 n2 = 1 then warning e1 e2;
                loop (e2 :: events)))
    | _ -> ()
  in
  loop events
...
let check_related_person_pevents warning _base birth_date death_date p iper
    related_p =
  List.iter
    (fun e ->
...
let check_related_on_person_update warning base birth_date death_date p iper
  ...
  check_related_person_pevents warning base birth_date death_date p iper
    related_p;
