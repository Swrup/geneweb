...
let commd_no_params conf =
  ...
  ^ List.fold_left
      (fun c (k, v) ->
        c ^ (if c = "" then "" else "&") ^ k ^ if v = "" then "" else "=" ^ v)
      "" conf.henv
...
let link_to_referer conf =
  ...
  if referer <> "" then
    Printf.sprintf
      "<a href=\"%s\"><span class=\"fa fa-arrow-left fa-lg\" \
       title=\"%s\"></span></a>\n"
      referer back
  else ""
...
let gen_print_link_to_welcome f conf right_aligned =
  ...
  Output.printf conf
    "<a href=\"%s\"><span class=\"fa fa-home fa-lg ml-1 px-0\" \
     title=\"%s\"></span></a>\n"
...
let header_without_http conf title =
  ...
  (match Util.open_templ conf "hed" with
...
let gen_trailer with_logo conf =
  ...
  (match Util.open_templ conf "trl" with
  | Some ic -> Templ.copy_from_templ conf [] ic
  | None -> ());
