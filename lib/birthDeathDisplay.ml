...
let print_longest_lived conf base =
  ...
  let title _ =
    Output.printf conf
      (fcapitale (ftransl conf "the %d who lived the longest"))
      len
  in
  Hutil.header conf title;
  Hutil.print_link_to_welcome conf true;
...
let old_print_statistics conf =
  let title _ =
    Output.print_string conf (Utf8.capitalize_fst (transl conf "statistics"))
  in
  let n =
  ...
  if conf.wizard || conf.friend then (
    ...
    Output.printf conf (ftransl conf "the latest %t deaths") (fun _ ->
        string_of_int n);
    Output.print_string conf "</a>";
    Output.print_string conf "</li>\n";
    Output.print_string conf "<li>";
...
let print_population_pyramid conf base =
  ...
  (let c = " cellspacing=\"0\" cellpadding=\"0\"" in
   ...
   for i = first_interv downto 0 do
     ...
     (if nb_men = 0 then ()
       ...
       Output.printf conf
         "<img src=\"images/pyr_male.png\" width=%d height=%d />" (n * 3) 14);
     Output.print_string conf "</td>\n";
     Output.print_string conf "</tr>\n";
     ...
     (if nb_wom = 0 then ()
       ...
       (* la largeur de l'image : 3 x 14     *)
       Output.printf conf
         "<img src=\"images/pyr_female.png\" width=%d height=%d />" (n * 3) 14);
     Output.print_string conf "</td>\n";
     Output.print_string conf "<td class=\"pyramid_nb\">";
     Output.print_string conf "&nbsp;";
  ...
  Output.printf conf "%s %s"
    (Utf8.capitalize_fst (transl conf "number of living persons:"))
  ...
  hidden_env conf;
  Output.print_string conf
    "<input type=\"hidden\" name=\"m\" value=\"POP_PYR\">\n";
