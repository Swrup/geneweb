...
let give_access conf base ia_asex p1 b1 p2 b2 =
  let reference _ _ p s =
    if is_hidden p then s
      ...
      ^ "&image="
      ^ (if p_getenv conf.env "image" = Some "off" then "off" else "")
      ^ "&bd="
      ^ (match p_getenv conf.env "bd" with Some x -> x | None -> "0")
      ^ "\">" ^ s ^ "</a>"
  in
      ...
      (fun (ip, ia_asex, rev_br) ->
        ...
        let with_sp =
          if Array.length (get_family p) = 1 then
            ...
            Printf.sprintf " %s %s" (Util.transl conf "with")
          ...
          if lev <= 2 then (
            ...
            if lev = 1 then (
              ...
              Output.printf conf "%s%s%s%s\n"
                (Utf8.capitalize_fst (Util.translate_eval s))
                with_sp (Util.transl conf ":")
                (if with_sp = "" then "<br>" else ""));
          (* the function children_of returns *all* the children of ip *)
          Array.iter
            (fun ifam ->
              let children =
                List.map
                  (fun ip -> (ip, ia_asex, (get_iper p, get_sex p) :: rev_br))
...
let print_cousins conf base p lev1 lev2 =
  let title h =
    ...
    if lev1 = lev2 then
      ...
      Output.printf conf "%s %d / %s %d"
        (Utf8.capitalize_fst (transl conf "ancestors"))
        lev1
...
let print_anniv conf base p dead_people level =
  ...
  let txt_of (up_sosa, down_br, spouse) conf base c =
    ...
    ^ acces_n conf base "2"
        (match spouse with Some ip -> pget conf base ip | _ -> c)
    ^ "&b2="
...
let print conf base p =
  let max_lev =
    ...
    with Not_found | Failure _ -> Perso.default_max_cousin_lev
  in
  match
