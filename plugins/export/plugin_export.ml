...
let export conf base =
  ...
  | Some output ->
      ...
      in
      let ipers =
        if List.assoc_opt "spouses" conf.env = Some "on" then
          IPS.fold
            (fun iper acc ->
              Array.fold_left
                (fun acc ifam ->
      ...
      in
      let ipers =
        if List.assoc_opt "parents" conf.env = Some "on" then
          IPS.fold
            (fun iper acc ->
      ...
      in
      let ipers =
        if List.assoc_opt "children" conf.env = Some "on" then
          IPS.fold
            (fun iper acc ->
              Array.fold_left
                (fun acc ifam ->
...
let () =
  Gwd_lib.GwdPlugin.register ~ns
    [ ("EXPORT", fun _assets -> w_base @@ w_lock @@ export) ]
