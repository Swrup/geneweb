...
and eval_message_string_var conf str so = function
  | [ "cut"; s ] -> (
      try VVstring (no_html_tags (sp2nbsp (int_of_string s) str))
...
let print_del_ok conf next_pos =
  let title _ =
    Output.print_string conf
      (Utf8.capitalize_fst (transl conf "message deleted"))
  in
...
let print_add_ok conf base =
  ...
  then visualize conf base mess
    ...
    let title _ =
      Output.print_string conf
        (Utf8.capitalize_fst (transl conf "message added"))
    in
